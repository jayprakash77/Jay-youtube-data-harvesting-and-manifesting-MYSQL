...
def comment_sql():
    try:
        ...
        try:
            ...
            pass
        comment_query = "INSERT IGNORE into comment VALUES(%s,%s,%s,%s,%s)"
        for i in item[0].keys():
            com =item[0][i]['comments']
            # print(len(com[0]))
            for m in com[0].keys():
                timestamp = com[0][m]['Comment_PublishedAt']
                for character in 'TZ':
                    timestamp = timestamp.replace(character, ' ')
                row = (str(com[0][m]['comment_Id']),str(com[0][m]['video_Id']),str(com[0][m]['comment_Text']),str(com[0][m]['commentAuthor']),str(timestamp))
                comment.append(row)
...
def stored_data(channel_ids):
    ...
    return "Data retriving and storing Done!!!"
